...
- **Test environments are throwaway Docker, and they are torn down.** Anything a check, test or
  ...
  running, or that there was none. `make verify-db` included: it stands up its own Postgres from
  `compose.verify.yml` and removes it again, on the failing path too. `skills/testing.md`.
